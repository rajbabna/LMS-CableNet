...
function pageTemplate(course, lesson, prev, next) {
  const styleV = fs.existsSync(path.join(ROOT, 'css', 'style.css'))
    ? 'css/style.css' : 'css/style.css';
  const navLinks = [];
  if (prev) navLinks.push(`      <a class="btn btn-ghost" href="${prev.file}">&larr; ${prev.label}</a>`);
  navLinks.push(`      <a class="btn btn-ghost" href="${course.page}">Back to course</a>`);
...
<head>
  ...
  <title>${esc(lesson.title)} — Cable&Net Courses</title>
...
<footer>© 2026 Raj Babna · Cable&amp;Net Courses</footer>
...
function emitSql(lessons) {
  ...
  for (const course of COURSES) {
    ...
    for (const l of list) {
      const url = `${LIVE_URL}/lesson-${course.id}-${String(l.moduleNumber).padStart(2, '0')}.html`;
  ...
  return `-- ===========================================================
-- Step 31: Course content — 18 real modules pointing at the
-- generated lesson pages.
-- GENERATED by tools/build-lessons.js — do not edit by hand.
-- ===========================================================
...
for (const course of COURSES) {
  ...
  parsed.forEach((l, idx) => {
    ...
    const outFile = path.join(ROOT, `lesson-${course.id}-${String(l.moduleNumber).padStart(2, '0')}.html`);
    ...
    console.log(`  wrote ${outFile.split(path.sep).pop()}`);
  });
...
console.log(totalWarnings ? `${totalWarnings} unclassified section(s) — check the pages visually` : 'All sections classified cleanly.');
