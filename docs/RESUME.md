...
**Last confirmed push:** `main`, commit `470b1e9` — clean tree, all synced.
...
- ✅ SQL/RPCs — schema through `sql/26`; RPCs: `add_student` (now with `p_expires_at`),
  `get_my_courses`, `get_course_instructors`, `assign_instructor`, `get_admin_overview`,
  `get_instructor_dashboard` (returns `expires_at`), `set_profile_role`
